...
export const CreateTaskSchema = z.object({
      ...
      basePriority: z.enum(['low', 'medium', 'high']).describe('The baseline urgency level of the item'),
    });
...
export const GetTasksSchema = z.object({
      startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).describe('Start date window in YYYY-MM-DD format'),
      endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).describe('End date window in YYYY-MM-DD format'),
      status: z.enum(['pending', 'completed', 'graveyard']).optional().describe('Filter results by specific status states'),
    })

export const UpdateTaskSchema = z.object({
      taskId: z.uuid().describe('The unique UUID identifier of the target task'),
      ...
      estimatedMinutes: z.number().int().positive().optional().describe('Revised execution duration in minutes'),
      scheduledDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().describe('Revised execution date in YYYY-MM-DD'),
      basePriority: z.enum(['low', 'medium', 'high']).optional().describe('Revised urgency priority status'),
    })

export const DeleteTaskSchema = z.object({
      taskId: z.uuid().describe('The unique UUID target identifier for deletion'),
    })